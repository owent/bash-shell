...
if [ "0" == $(is_in_list llvm $BUILD_TARGET_COMPOMENTS) ]; then
    LLVM_PKG=$(check_and_download "llvm" "llvm-*.tar.xz" "http://llvm.org/releases/3.5.1/llvm-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$LLVM_PKG";
        exit -1;
    fi
...
if [ "0" == $(is_in_list clang $BUILD_TARGET_COMPOMENTS) ]; then
    CLANG_PKG=$(check_and_download "clang" "cfe-*.tar.xz" "http://llvm.org/releases/3.5.1/cfe-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$CLANG_PKG";
        exit -1;
    fi
...
if [ "0" == $(is_in_list clang_tools_extra $BUILD_TARGET_COMPOMENTS) ]; then
    CLANG_TOOLS_EXTRA_PKG=$(check_and_download "clang tools extra" "clang-tools-extra-*.tar.xz" "http://llvm.org/releases/3.5.1/clang-tools-extra-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
...
if [ "0" == $(is_in_list compiler_rt $BUILD_TARGET_COMPOMENTS) ]; then
    COMPILER_RT_PKG=$(check_and_download "compiler rt" "compiler-rt-*.tar.xz" "http://llvm.org/releases/3.5.1/compiler-rt-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
...
if [ "0" == $(is_in_list lldb $BUILD_TARGET_COMPOMENTS) ]; then
    LLDB_PKG=$(check_and_download "lldb" "lldb-*.tar.xz" "http://llvm.org/releases/3.5.1/lldb-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$LLDB_PKG";
        exit -1;
    fi
...
if [ "0" == $(is_in_list lld $BUILD_TARGET_COMPOMENTS) ]; then
    LLD_PKG=$(check_and_download "lld" "lld-*.tar.xz" "http://llvm.org/releases/3.5.1/lld-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$LLD_PKG";
        exit -1;
    fi
...
if [ "0" == $(is_in_list polly $BUILD_TARGET_COMPOMENTS) ]; then
    POLLY_PKG=$(check_and_download "polly" "polly-*.tar.xz" "http://llvm.org/releases/3.5.1/polly-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$POLLY_PKG";
        exit -1;
    fi
...
if [ ! -z "$BUILD_GCC" ] && [ "0" == $(is_in_list dragonegg $BUILD_TARGET_COMPOMENTS) ]; then
    DRAGONEGG_PKG=$(check_and_download "dragonegg" "dragonegg-*.tar.xz" "http://llvm.org/releases/3.5.1/dragonegg-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
...
if [ "0" == $(is_in_list libcxx $BUILD_TARGET_COMPOMENTS) ]; then
    LIBCXX_PKG=$(check_and_download "libc++" "libcxx-*.tar.xz" "http://llvm.org/releases/3.5.1/libcxx-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$LIBCXX_PKG";
        exit -1;
    fi
...
if [ "0" == $(is_in_list libcxxabi $BUILD_TARGET_COMPOMENTS) ]; then
    LIBCXXABI_PKG=$(check_and_download "libc++abi" "libcxxabi-*.tar.xz" "http://llvm.org/releases/3.5.1/libcxxabi-3.5.1.src.tar.xz" );
    if [ $? -ne 0 ]; then
