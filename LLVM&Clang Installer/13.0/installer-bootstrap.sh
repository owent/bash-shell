...
COMPOMENTS_PYTHON_VERSION=3.9.7
...
function build_llvm_toolchain() {
  ...
  if [[ ! -z "$BUILD_USE_GCC_TOOLCHAIN" ]]; then
    ...
    STAGE_BUILD_EXT_COMPILER_FLAGS=("${STAGE_BUILD_EXT_COMPILER_FLAGS[@]}"
      ...
      "-DLIBUNWIND_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
      "-DLIBCXXABI_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
      "-DLIBCXX_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
      "-DBOOTSTRAP_LIBUNWIND_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
      "-DBOOTSTRAP_LIBCXXABI_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
      "-DBOOTSTRAP_LIBCXX_GCC_TOOLCHAIN=$BUILD_USE_GCC_TOOLCHAIN"
    )
  fi
