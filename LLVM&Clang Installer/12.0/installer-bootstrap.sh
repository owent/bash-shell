...
BUILD_LLVM_LLVM_OPTION="$BUILD_LLVM_LLVM_OPTION -DLLVM_ENABLE_PIC=ON -DLLVM_USE_INTEL_JITEVENTS=ON -DLLVM_ENABLE_MODULES=ON -DLLVM_ENABLE_LTO=ON"
...
function build_llvm_toolchain() {
    ...
    if [[ ! -z "$BUILD_USE_GCC_TOOLCHAIN" ]]; then
        ...
        STAGE_BUILD_EXT_COMPILER_FLAGS="$STAGE_BUILD_EXT_COMPILER_FLAGS '-DCMAKE_CXX_FLAGS=$CMAKE_CXX_FLAGS' '-DCMAKE_C_FLAGS=$CMAKE_C_FLAGS'"
    fi
    ...
    if [[ ! -z "$CMAKE_RUNTIME_LINKER_FLAGS" ]]; then
        ...
        STAGE_BUILD_EXT_COMPILER_FLAGS="$STAGE_BUILD_EXT_COMPILER_FLAGS $CMAKE_RUNTIME_LINKER_FLAGS"
    fi

    STAGE_BUILD_CMAKE_OPTION=""
    if [[ "x$BUILD_TARGET_COMPOMENTS" == "xall" ]]; then
        STAGE_BUILD_CMAKE_OPTION="-DCOMPILER_RT_USE_BUILTINS_LIBRARY=ON -DLIBUNWIND_USE_COMPILER_RT=ON -DLIBCXX_USE_COMPILER_RT=ON -DLIBCXXABI_USE_COMPILER_RT=ON -DLIBCXXABI_USE_LLVM_UNWINDER=ON"
        ...
        if [[ $BUILD_TARGET_HAS_COMPILER_RT -ne 0 ]] && [[ $BUILD_TARGET_HAS_LIBCXX -ne 0 ]] && [[ $BUILD_TARGET_HAS_LIBCXX_ABI -ne 0 ]] && [[ $BUILD_TARGET_HAS_LIBUNWIND -ne 0 ]] && [[ $BUILD_TARGET_HAS_LLD -ne 0 ]]; then
            STAGE_BUILD_CMAKE_OPTION="$STAGE_BUILD_CMAKE_OPTION -DCOMPILER_RT_USE_BUILTINS_LIBRARY=ON"
        fi
