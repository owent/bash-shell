...
# ======================================= 配置 ======================================= 
PREFIX_DIR=/usr/local/gcc-5.1.0
...
if [ -z "$BUILD_TARGET_COMPOMENTS" ] || [ "0" == $(is_in_list mpfr $BUILD_TARGET_COMPOMENTS) ]; then
    MPFR_PKG=$(check_and_download "mpfr" "mpfr-*.tar.xz" "http://www.mpfr.org/mpfr-current/mpfr-3.1.2.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$MPFR_PKG";
        exit -1;
    fi
...
if [ -z "$BUILD_TARGET_COMPOMENTS" ] || [ "0" == $(is_in_list gdb $BUILD_TARGET_COMPOMENTS) ]; then
    if [ -z "$(whereis ncurses | awk '{print $2;}')" ]; then
	    ...
	    if [ ! -z "$(find /usr/include -name Python.h)" ]; then
		    ...
		    # =======================  尝试编译安装python  =======================
		    PYTHON_PKG=$(check_and_download "python" "Python-2.*.tar.xz" "https://www.python.org/ftp/python/2.7.9/Python-2.7.9.tar.xz" );
		    if [ $? -ne 0 ]; then
			    return;
		    fi
	    ...
	    # ======================= 正式安装GDB =======================
	    GDB_PKG=$(check_and_download "gdb" "gdb-*.tar.xz" "http://ftp.gnu.org/gnu/gdb/gdb-7.9.tar.xz" );
	    if [ $? -ne 0 ]; then
		    echo -e "$GDB_PKG";
		    exit -1;
	    fi
