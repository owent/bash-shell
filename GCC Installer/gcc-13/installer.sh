...
COMPOMENTS_LIBEXPAT_VERSION=2.6.1
...
function build_xz() {
  ...
  if [[ -z "$BUILD_TARGET_COMPOMENTS" ]] || [[ "0" == $(is_in_list xz $BUILD_TARGET_COMPOMENTS) ]]; then
    XZ_PKG=$(check_and_download "xz" "xz-*.tar.gz" "https://github.com/tukaani-project/xz/releases/download/v$COMPOMENTS_XZ_VERSION/xz-$COMPOMENTS_XZ_VERSION.tar.gz" "xz-$COMPOMENTS_XZ_VERSION.tar.gz")
    if [[ $? -ne 0 ]]; then
      echo -e "$XZ_PKG"
      exit 1
    fi
