...
COMPOMENTS_GCC_VERSION=10.1.0;
COMPOMENTS_BINUTILS_VERSION=2.34;
COMPOMENTS_PYTHON_VERSION=2.7.18;
COMPOMENTS_GDB_VERSION=9.1;
COMPOMENTS_GLOBAL_VERSION=6.6.4;
COMPOMENTS_ZSTD_VERSION=1.4.4;
COMPOMENTS_LZ4_VERSION=1.9.2;
...
while getopts "dp:cht:d:g:n" OPTION; do
    case $OPTION in
        ...
        h)
            ...
            echo "-h                          help message.";
            echo "-t [build target]           set build target(gmp mpfr mpc isl zstd lz4 gcc binutils gdb libatomic_ops bdw-gc global).";
...
if [ -z "$BUILD_TARGET_COMPOMENTS" ] || [ "0" == $(is_in_list global $BUILD_TARGET_COMPOMENTS) ]; then
    ...
    if [ $BUILD_DOWNLOAD_ONLY -eq 0 ]; then
        ...
        # patch for global 6.6.4 linking error
        grep "^int\\s*type" libutil/args.c;
        if [ $? -eq 0 ]; then
            grep "^int\\s*type" global/global.c;
            if [ $? -eq 0 ]; then
                sed -i "s;^int\\s*type;extern int type;" global/global.c
            fi
        fi
        make $BUILD_THREAD_OPT && make install;
        cd "$WORKING_DIR";
